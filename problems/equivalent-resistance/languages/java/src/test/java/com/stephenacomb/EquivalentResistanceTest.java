...
public class EquivalentResistanceTest {

	private static float points = 0;
	private static final double DELTA = 1e6;

	private double[] exampleBaseResistances() {
		return new double[] {
	...
	@Test
	public void test1() {
		double[] baseResistances = exampleBaseResistances();
	...
	public void test3() {
		...
		double expected = ResistorUtils.evaluateConfig(config, baseResistances);
		double actual = ResistorUtils.evaluateConfig(
				solution.approximate(baseResistances, 0, maxResistors), baseResistances);
		assertEquals(expected, actual, (expected/DELTA));
		points += 1.0;
	}
	...
	public static void showPoints() {
		System.out.printf("EQUIVALENT_RESISTANCE POINTS = %.1f of 8.0\n", points);
	}
